...
func (s *ServiceBusCli) CreateSubscriptionRule(subscription SubscriptionEntity, rule RuleEntity) error {
	...
	logger.LogHighlight("Subscription rule %v was created successfully for subscription %v on topic %v in service bus %v", log.Error, rule.Name, subscription.Name, subscription.TopicName, s.Namespace.Name)
